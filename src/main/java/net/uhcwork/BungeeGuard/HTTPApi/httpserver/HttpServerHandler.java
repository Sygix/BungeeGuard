...
public class HttpServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    private HttpResponse getResponse(ChannelHandlerContext channelHandlerContext, HttpRequest hr) {
        ...
        final RequestHandler handler = HttpApi.getRequestManager().getHandlerForEndpoint(query.path());
        if (handler == null) {
            reply = "No such endpoint exists.";
            hrs = HttpResponseStatus.NOT_FOUND;
        } else {
            final HttpServerApiRequest ar = new HttpServerApiRequest(((InetSocketAddress) channelHandlerContext.channel().remoteAddress()).getAddress(),
                    params, bodyBuilder.toString());
            try {
                if (handler.requiresAuthentication() && !HttpApi.getSelf().getAuthenticationProvider().authenticate(ar, query.path())) {
                ...
                reply = "An internal error has occurred. Information has been logged to the console.";
                System.out.println("Error while handling " + hr.getUri() + " from " + ar.getRemoteIp());
                throwable.printStackTrace();
            }
        }

        String json = Main.getGson().toJson(reply);
        ...
        hreply.headers().set("Server", "BungeeGuard/0.1");
