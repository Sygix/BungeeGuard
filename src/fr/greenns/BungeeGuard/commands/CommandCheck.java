    ...
    public void execute(CommandSender sender, String[] args)
    {
        ...
        if(args.length >= 1 )
        {
            ...
            try {
                ...
                String safeuuid = BungeeCord.getInstance().getPlayer(args[0]).getUUID().toString().toLowerCase().replaceAll("'", "\"");

                res = plugin.sql.query("SELECT COUNT(id) as count FROM `BungeeGuard_Ban` WHERE `uuidBanned` = '"+safeuuid+"'");

                if (res.next())
                {
                    message = "Le joueur "+args[0]+" a "+ res.getInt("count") +" ban";
                    activeBans = plugin.sql.query("SELECT COUNT(id) as count FROM `BungeeGuard_Ban` WHERE `uuidBanned` = '"+safeuuid+"' AND `status` = 1");
                    if ( activeBans.next())
                    {
