...
public class CommandKick extends Command {
	...
	public void execute(CommandSender sender, String[] args)
	{
		...
		if(BungeeCord.getInstance().getPlayer(args[0]) != null)
		{
			...
			msg = nick + " a kické " + target.getDisplayName() + " pour : " + powodd;

			System.out.println("§c"+msg);

			for (ProxiedPlayer playerdwa : target.getServer().getInfo().getPlayers())
			{
				if(playerdwa.hasPermission("bungeeguard.notify"))
				{
					playerdwa.sendMessage(plugin.utils.staffBroadcast + ChatColor.RED +msg);      
				}
			}
			sender.sendMessage("§a" + target.getName() + " §2a été kické !");
			target.disconnect(kickmsg);
		}
		else if (BungeeCord.getInstance().getPlayer(args[0]) == null)
		{
			sender.sendMessage("§c§o" + args[0] + "§r§c n'est pas connécté !");
		}
	}
