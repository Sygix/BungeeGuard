	...
	public void execute(CommandSender sender, String[] args) {
		...
		else
		{
			...
			if (plugin.spy.contains(p.getUniqueId())) {
				...
				p.sendMessage(new ComponentBuilder("Vous avez désactivé ").color(ChatColor.GRAY)
						.append("SpyChat").color(ChatColor.RED).create());
			} else {
				plugin.spy.add(p.getUniqueId());
				p.sendMessage(new ComponentBuilder("Vous avez activé ").color(ChatColor.GRAY)
						.append("SpyChat").color(ChatColor.GREEN).create());
			}
		}
