...
public class CommandUnban extends Command {
	...
	public void execute(CommandSender sender, String[] args)
	{
		...
		if(args.length >= 1)
		{
			...
			try
			{
				ResultSet res = plugin.sql.query("SELECT id FROM `BungeeGuard_Ban` WHERE `nameBanned` = '"+safenick+"' and `status` = 1");
				//res.last();
				...
				if(activeBan==true)
				{
					...
					msg = "§a"+safenick+"§c a été débanni par §7"+unbanName;
				...
				{
					sender.sendMessage("§c"+safenick+" joueur n'est pas banni ...");
				}
